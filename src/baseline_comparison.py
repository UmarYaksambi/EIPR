...
from .attack_suite import build_baseline_attack_suite


# ---------------------------------------------------------------------------
# Baseline watermarkers
# ---------------------------------------------------------------------------
...
class FixedRateWatermarker:
    ...
    def embed(
        ...
        return (
            embed_watermark(image_bgr, watermark_bits, rate_map, self._engine, scheme, alpha=alpha),
            rate_map,
        )
    ...
    def extract(
        ...
        return extract_watermark(
            image_bgr, rate_map, self._engine, n_bits,
            scheme=scheme, alpha=alpha, original_bgr=original_bgr,
        )


class VarianceAdaptiveDCT:
    """
    Simple two-tier variance-adaptive DCT watermarker used as an internal
    ablation baseline.  This is NOT a re-implementation of any published paper.
    Embed only in the top-50% variance blocks at a fixed rate=0.50; skip the
    rest.  It serves to isolate the contribution of three-tier adaptive ECC
    over a basic high-texture-only strategy.
    """
    def __init__(self, alpha: float = ALPHA):
        self.alpha = float(alpha)
        self._engine = AdaptiveECCEngine()
        self._last_rate_map: np.ndarray | None = None

    def embed(self, image_bgr: np.ndarray, watermark_bits: np.ndarray) -> np.ndarray:
        ycrcb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2YCrCb)
        var_map = compute_block_dct_variance(ycrcb[:, :, 0])
        tau = float(np.percentile(var_map, 50))
    ...
    def extract(
        ...
        return extract_watermark(
            image_bgr, self._last_rate_map, self._engine, n_bits,
            alpha=self.alpha, original_bgr=original_bgr,
        )
...
class LSBWatermarker:
    def embed(self, image_bgr: np.ndarray, watermark_bits: np.ndarray) -> np.ndarray:
        ...
        if n > len(Y):
            raise ValueError(f"LSB capacity = {len(Y)} bits but watermark has {n} bits.")
...
class SSWatermarker:
    ...
    def _mid_freq_indices(self, flat_size: int, n_coeffs: int) -> np.ndarray:
        ...
        mid_end = flat_size // 8 + n_coeffs * 4
...
def run_baseline_comparison(
    ...
    """
    Run all baselines against the provided attack set.

    Unique watermark payload per image
    ------------------------------------
    Each image i receives watermark derived from RNG(seed + i).  This prevents
    a reviewer objection that BER results are payload-dependent.  All methods
    receive the SAME per-image watermark so results are directly comparable.

    Baselines
    ----------
    adaptive_ecc          — proposed method (three-tier adaptive ECC + sync tones)
    variance_adaptive_dct — two-tier high-texture-only DCT (internal ablation)
    fixed_rate_0.25/0.50/0.75 — fixed ECC rate ablations
    lsb                   — least-significant-bit spatial domain
    spread_spectrum       — informed (non-blind) SS in DCT domain
    spread_spectrum_blind — blind SS (no original reference at decode)
    """
    try:
    ...
    require_real = bool((cfg.get("attacks") or {}).get("require_real_regeneration", False))
    if attacks is None:
    ...
    alpha = float((cfg.get("embedding") or {}).get("alpha", ALPHA))

    lsb_bl   = LSBWatermarker()
    ss_bl    = SSWatermarker()
    sota_bl  = VarianceAdaptiveDCT(alpha=alpha)
    fixed_bls = {
    ...
    attack_iter = (
        tqdm(attacks.items(), desc="Attacks")
        if _tqdm_available else attacks.items()
    )
    ...
    for attack_name, attack_fn in attack_iter:
        ...
        # enumerate so we can derive per-image watermark
        raw_iter = list(enumerate(images))
        img_iter = (
            tqdm(raw_iter, desc=f"  {attack_name}", leave=False)
            if _tqdm_available else raw_iter
        )

        for i, img in img_iter:
            # Unique payload per image — same for all methods on this image
            watermark = np.random.default_rng(seed + i).integers(0, 2, n_bits).astype(np.uint8)
            ...
            dec = extract_watermark(
                ...
                scheme=scheme, alpha=alpha, original_bgr=wm,
            )
            adap_bers.append(bit_error_rate(watermark, dec))
            adap_psnrs.append(image_psnr(img, wm))

            # 2. Variance-adaptive DCT (internal ablation baseline)
        ...
        all_results["variance_adaptive_dct"][attack_name] = _stats(sota_bers)
        for key in fixed_bls:
            all_results[key][attack_name] = _stats(fixed_bers[key])
        all_results["lsb"][attack_name]                  = _stats(lsb_bers)
        all_results["spread_spectrum"][attack_name]      = _stats(ss_bers)
        all_results["spread_spectrum_blind"][attack_name] = _stats(ss_blind_bers)
