...
from .watermark_embedder import ALPHA, BLOCK_SIZE, BITS_PER_BLOCK, EMBED_COEFF_INDICES, _decode_coeff
from .geometric_sync import correct_attacked_image

def extract_watermark(
    image_bgr: np.ndarray, n_bits: int, ecc_engine: AdaptiveECCEngine,
    rate_map: np.ndarray, scheme: ECCScheme = "reed_solomon", alpha: float = ALPHA,
) -> np.ndarray:
    # 1. Blind Geometric Correction (thresholded heavily)
    ...
    s_r, s_c = Y.strides
    blocks = as_strided(Y, shape=(n_rows, n_cols, BLOCK_SIZE, BLOCK_SIZE), strides=(s_r * BLOCK_SIZE, s_c * BLOCK_SIZE, s_r, s_c))
    dct_flat: np.ndarray = np.asarray(dctn(np.ascontiguousarray(blocks), norm="ortho", axes=(-2, -1))).reshape(n_rows, n_cols, BLOCK_SIZE * BLOCK_SIZE)
    ...
    for tier_rate in unique_rates:
        ...
        decoded, success = ecc_engine.decode_block(raw_codeword, tier_rate, scheme, n_payload=n_bits)
        weight = tier_rate * (100.0 if success else 1.0)
        tier_results.append((decoded, weight))

    if not tier_results: return np.zeros(n_bits, dtype=np.uint8)
    total_w = sum(w for _, w in tier_results)
    if total_w == 0: return np.zeros(n_bits, dtype=np.uint8)
