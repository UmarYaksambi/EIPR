...
class AdaptiveECCEngine:
    def encode_block(self, payload_bits: np.ndarray, ecc_rate: float, scheme: ECCScheme = "reed_solomon") -> np.ndarray:
    ...
    def decode_block(self, received_bits: np.ndarray, ecc_rate: float, scheme: ECCScheme = "reed_solomon", n_payload: int | None = None) -> tuple[np.ndarray, bool]:
        ecc_rate = float(np.clip(ecc_rate, 0.0, 0.99))
        ...
        if scheme == "reed_solomon":
            ...
            if len(clipped) < usable:
                clipped = np.concatenate([clipped, np.zeros(usable - len(clipped), dtype=np.uint8)])
    ...
    def _rs_decode(self, bits: np.ndarray, ecc_rate: float, n_payload: int) -> tuple[np.ndarray, bool]:
        ...
        try:
            ...
            return np.unpackbits(np.frombuffer(decoded_bytes, dtype=np.uint8))[:n_payload], True
